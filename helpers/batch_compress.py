...
Progress -> reports/batch_progress.csv (flushed per root).
  ...
  python helpers/batch_compress.py [--skip FOLDER ...] [--tessdata DIR]
...
REPO = Path(__file__).resolve().parent.parent   # this script lives in helpers/
PY = sys.executable
...
for i, root in enumerate(roots, 1):
    ...
    try:
        r = subprocess.run(
            [PY, TOOL, root, "--in-place", "--language", "auto"],
